...
def train_and_save(model, train_in, train_out, valid_in=None, valid_out=None, weights_file=None, model_file=None):
    ...
    model.fit(train_in, train_out, callbacks=cb, validation_data=(valid_in, valid_out), nb_epoch=20)
...
def predict():
    ...
    valid_headless_out, valid_head_expected_out, number_of_classes, valid_filenames = get_base_predictions(base_model=base_model, results_path=results_path, directory_to_search=validation_path)
    test_headless_out, test_head_expected_out, number_of_test_classes, test_filenames = get_base_predictions(base_model=base_model, results_path=results_path, directory_to_search=test_path)
    ...
    create_submission_file_for_state_farm(test_head_predicted_out, test_filenames, sample_submission_file, output_submission_file)
